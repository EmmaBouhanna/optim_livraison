...
cost_dist = 1/3600  # unit time cost
...
def one_client_to_deliver(file):
    ...
    for i in range(1, len(file)//3 + 1):
        if file[3*i-1] == 1:
            instance = pd.read_csv(os.path.join(
                PATH, 'input_data', file[3*(i-1)]))
            columns_res = ['Camion 1']
            res = [instance['Identifiant'][i] for i in [0, 1, 0]]
            print(res)
            res = pd.DataFrame(res, columns=columns_res)
            res.to_csv(os.path.join(PATH, 'output_data',
                                    'res_entrepot_' + str(instance['Identifiant'][0]) + '.csv'))
        else:
...
def ind2route(individual, instance, distance_matrix, vehicle_capacity, max_vehicle, initCost=0.0, unitCost=cost_dist, serviceTime=service_time):
    ...
    for customerID in individual:
        ...
        # Update elapsed time
        # distance_matrix = distance entre i et j
        returnTime = distance_matrix[customerID][0]*unitCost
        updatedElapsedTime = elapsedTime + \
            distance_matrix[lastCustomerID][customerID] * \
            unitCost + serviceTime + returnTime
...
def evalVRPTW(individual, instance, distance_matrix, vehicle_capacity, max_vehicle, unitCost=cost_dist, initCost=0.0):
    ...
    for subRoute in route:
        ...
        subRouteDistance = subRouteDistance + \
            distance_matrix[lastCustomerID][0]
...
def mut_inverse_indexes(individual):
    '''
    Step of genetic algorithm : Mutation (only by inversion, to keep the unicity of each occurence) . 
    No insertion or deletion allowed
    ...
    individual = individual[:start + 1] + \
        individual[stop:start:-1] + individual[stop+1:]
    return (individual, )
...
def run_vrptw(instance, distance_matrix, vehicle_capacity, max_vehicle, ind_size, pop_size,
    ...
    # Positive weights because we intend to maximise fitness
    ...
    # Cost Evaluation
    toolbox.register('evaluate', evalVRPTW, instance=instance, distance_matrix=distance_matrix,
                     vehicle_capacity=vehicle_capacity, max_vehicle=max_vehicle, unitCost=unit_cost, initCost=init_cost)
    ...
    # Keep track of each indiviual's cost as attributes of creator Individual
    ...
    for gen in range(n_gen):
        ...
        # Every individual is able to move on to the next generation
        ...
        pop[:] = offspring
        # returns a list containing the k best individuals among the population, here the best one
        best_ind = tools.selBest(pop, 1)[0]
        print(
            f'Best Fitness at Generation {gen}: {best_ind.fitness.values[0]}')
...
def decode_to_GPS(liste_res, instances):
    ...
    for (routes_warehouse, instance) in zip(liste_res, instances):
        ...
        columns_res = ['camion' + str(k+1)
                       for k in range(len(routes_warehouse))]
...
def simulation_vrptw(garage, truck, number_clients):
    ...
    if G.matrix is not None:
        np.savetxt(os.path.join(
            PATH, 'output_data_bis/corrected_travel_times_array.csv'), G.matrix, delimiter=",")
    ...
    instances = []  # list to put together the results for each warehouse
    liste_res = []
    for (i, file) in enumerate(file_properties[::3]):
        ...
        instance_bis = instance.drop(
            ['Unnamed: 0', 'Identifiant', 'latitude', 'longitude'], axis='columns')
        n = instance_bis.shape[1]
        number_of_points = n - 1
        number_of_clients = number_clients_per_warehouse[i]
        instance_bis.columns = ['demand'] + \
            [i for i in range(number_of_points)]
        distance_matrix = instance_bis[[i for i in range(
            0, number_of_points)]]  # columns matrix
        res = run_vrptw(instance_bis, distance_matrix, vehicle_capacity,
                        max_vehicle, number_of_clients, 100, 0.4, 0.2, 200)
